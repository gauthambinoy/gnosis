...
def upgrade() -> None:
    ...
    # --- agents ---
    agent_status = postgresql.ENUM("active", "idle", "paused", "error", "learning", name="agentstatus", create_type=True)
    ...
    # --- executions ---
    exec_status = postgresql.ENUM("queued", "running", "completed", "failed", "cancelled", "awaiting_approval", name="executionstatus", create_type=True)
    ...
    # --- memories ---
    mem_tier = postgresql.ENUM("correction", "episodic", "semantic", "procedural", name="memorytier", create_type=True)
