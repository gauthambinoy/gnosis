...
async def test_register_and_login(client, api_prefix):
    ...
    # Login
    r = await client.post(
        f"{api_prefix}/auth/login", json={"email": email, "password": "TestPass123!"}
    )
    assert r.status_code == 200
...
async def test_register_weak_password_rejected(client, api_prefix):
    r = await client.post(
        f"{api_prefix}/auth/register",
        json={"email": "weak@test.com", "password": "short", "full_name": "W"},
    )
...
async def test_agent_crud(client, api_prefix):
    ...
    r = await client.post(
        f"{api_prefix}/auth/register",
        json={"email": email, "password": "TestPass123!", "full_name": "CRUD"},
    )
    token = r.json()["access_token"]
    ...
    r = await client.patch(
        f"{api_prefix}/agents/{agent_id}", json={"name": "Updated"}, headers=headers
    )
    assert r.status_code == 200
...
async def test_login_wrong_password(client, api_prefix):
    ...
    await client.post(
        f"{api_prefix}/auth/register",
        json={"email": email, "password": "GoodPass123!", "full_name": "X"},
    )
    r = await client.post(
        f"{api_prefix}/auth/login", json={"email": email, "password": "WrongPass999!"}
    )
