...
class GmailConnector(BaseConnector):
    ...
    async def list_messages(
        ...
        async with session.get(
            f"{GMAIL_API}/messages", headers=headers, params=params
        ) as resp:
        ...
        for msg_stub in messages:
            async with session.get(
                f"{GMAIL_API}/messages/{msg_stub['id']}",
                headers=headers,
                params={"format": "full"},
            ) as resp2:
        ...
        try:
            dispatch = {
                "list_messages": lambda: self.list_messages(
                    inputs["user_id"],
                    inputs.get("query", ""),
                    inputs.get("max_results", 10),
                ),
                "get_message": lambda: self.get_message(
                    inputs["user_id"], inputs["message_id"]
                ),
                "send_email": lambda: self.send_message(
                    inputs["user_id"], inputs["to"], inputs["subject"], inputs["body"]
                ),
                "reply_message": lambda: self.reply_message(
                    inputs["user_id"], inputs["message_id"], inputs["body"]
                ),
