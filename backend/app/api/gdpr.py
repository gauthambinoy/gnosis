...
@router.post("/erasure")
async def request_erasure(reason: str = "", categories: Optional[List[str]] = None, user_id: str = Depends(get_current_user_id)):
    request = gdpr_engine.create_erasure_request(
        user_id=user_id, requested_by=user_id, reason=reason, data_categories=categories,
    )
