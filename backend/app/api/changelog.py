...
@router.get("")
async def list_changelog(limit: int = 20, category: Optional[str] = None, user_id: str = Depends(get_current_user_id)):
...
async def add_entry(data: dict, user_id: str = Depends(get_current_user_id)):
    return changelog_engine.add_entry(version=data["version"], title=data["title"], description=data.get("description", ""), category=data.get("category", "feature"), tags=data.get("tags", []))
