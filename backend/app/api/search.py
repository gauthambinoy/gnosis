...
async def global_search(
    ...
    try:
        ...
        for agent_id, agent in list(marketplace_engine._agents.items())[:1000]:
            name = (
                getattr(agent, "name", str(agent.get("name", "")))
                if isinstance(agent, dict)
                else getattr(agent, "name", "")
            )
            desc = (
                getattr(agent, "description", str(agent.get("description", "")))
                if isinstance(agent, dict)
                else getattr(agent, "description", "")
            )
    ...
    try:
        ...
        for pid, pipeline in list(pipeline_engine._pipelines.items()):
            if (
                query_lower in pipeline.name.lower()
                or query_lower in pipeline.description.lower()
            ):
                results["pipelines"].append(
                    {"id": pid, "name": pipeline.name, "type": "pipeline"}
                )
    except Exception:
        pass
    ...
    try:
        ...
        for fid, record in list(file_manager._files.items()):
            if query_lower in record.original_name.lower() or any(
                query_lower in t.lower() for t in record.tags
            ):
                results["files"].append(
                    {"id": fid, "name": record.original_name, "type": "file"}
                )
    except Exception:
        pass
