...
async def create_snapshot(agent_id: str, config: dict, description: str = "", user_id: str = Depends(get_current_user_id)):
    snapshot = config_snapshot_store.create_snapshot(agent_id, config, created_by=user_id, description=description)
...
async def activate_snapshot(snapshot_id: str, user_id: str = Depends(get_current_user_id)):
