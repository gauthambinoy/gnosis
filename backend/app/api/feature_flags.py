...
async def create_flag(data: dict, user_id: str = Depends(get_current_user_id)):
    return feature_flag_engine.create_flag(
        name=data["name"],
        description=data.get("description", ""),
        scope=data.get("scope", "global"),
        rollout_pct=data.get("rollout_pct", 100.0),
    )


@router.put("/{flag_id}")
async def update_flag(
    flag_id: str, data: dict, user_id: str = Depends(get_current_user_id)
):
...
async def check_flag(
    ...
    return {
        "name": name,
        "enabled": feature_flag_engine.is_enabled(
