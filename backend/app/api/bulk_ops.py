...
async def bulk_agent_action(
    req: BulkAgentAction, user_id: str = Depends(get_current_user_id)
):
        ...
        try:
            ...
            if req.action == "pause":
                ...
                results["errors"].append(
                    {"agent_id": agent_id, "error": f"unknown action: {req.action}"}
                )
                results["failed"] += 1
                continue
    ...
    logger.info(
        f"Bulk {req.action}: {results['succeeded']}/{results['total']} succeeded"
    )
    return results
...
async def bulk_retry_executions(
    req: BulkRetryRequest, user_id: str = Depends(get_current_user_id)
):
    """Bulk retry failed executions."""
    return {
        "total": len(req.execution_ids),
        "queued": len(req.execution_ids),
        "note": "Retries queued for processing",
    }
