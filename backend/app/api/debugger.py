...
async def list_debug_sessions(
    ...
    return {
        "sessions": time_travel_debugger.list_sessions(agent_id=agent_id, limit=limit)
    }
...
async def get_frames(
    ...
    return {
        "frames": time_travel_debugger.get_frames_range(
...
async def get_frame(
    session_id: str, frame_index: int, user_id: str = Depends(get_current_user_id)
):
...
async def search_frames(
    ...
    return {
        "frames": time_travel_debugger.search_frames(
            session_id, phase=phase, label_contains=label
        )
    }
