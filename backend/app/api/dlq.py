...
@router.get("")
async def list_entries(operation: Optional[str] = None, status: Optional[str] = None, limit: int = 50, user_id: str = Depends(get_current_user_id)):
    return {"entries": dead_letter_queue.list_entries(operation=operation, status=status, limit=limit)}
