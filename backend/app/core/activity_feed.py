...
class ActivityEvent:
    ...
    event_type: str = ""  # "agent.created", "execution.completed", "pipeline.failed", etc.
    title: str = ""
    description: str = ""
...
class ActivityFeed:
    ...
    def publish(self, workspace_id: str, event_type: str, title: str, actor_id: str = "", actor_type: str = "user", description: str = "", metadata: dict = None, mentions: list = None) -> ActivityEvent:
        event = ActivityEvent(
    ...
    def get_feed(self, workspace_id: str, limit: int = 50, event_type: str = None, after: str = None) -> List[dict]:
        events = self._events.get(workspace_id, [])
        if event_type:
            events = [e for e in events if e.event_type == event_type]
        if after:
            events = [e for e in events if e.timestamp > after]
        return [asdict(e) for e in sorted(events, key=lambda e: e.timestamp, reverse=True)[:limit]]

    def get_mentions(self, workspace_id: str, user_id: str, unread_only: bool = True) -> List[dict]:
        events = self._events.get(workspace_id, [])
        mentioned = [e for e in events if user_id in e.mentions]
        if unread_only:
            mentioned = [e for e in mentioned if user_id not in e.read_by]
        return [asdict(e) for e in sorted(mentioned, key=lambda e: e.timestamp, reverse=True)]
