...
class SandboxEngine:
    ...
    def create_session(self, user_id: str, ttl_minutes: int = 30) -> dict:
        session = SandboxSession(user_id=user_id, expires_at=(datetime.now(timezone.utc) + timedelta(minutes=ttl_minutes)).isoformat())
    ...
    def execute_in_sandbox(self, session_id: str, action: dict) -> dict:
        ...
        return {"action": action, "result": "simulated", "actions_remaining": session.max_actions - session.actions_count}
