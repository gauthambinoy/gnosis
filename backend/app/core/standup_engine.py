...
class StandupEngine:
    ...
    async def generate(self) -> dict:
        ...
        # Execution stats
        exec_started = [
            e for e in recent_entries if e["event_type"] == "execution.started"
        ]
        exec_completed = [
            e for e in recent_entries if e["event_type"] == "execution.completed"
        ]
        exec_failed = [
            e for e in recent_entries if e["event_type"] == "execution.failed"
        ]
        ...
        for entry in exec_completed:
            ...
            if aid in agent_stats:
                ...
                agent_stats[aid]["total_latency_ms"] += details.get(
                    "total_latency_ms", 0.0
                )
        ...
        notable: list[dict] = []
        trust_changes = [
            e for e in recent_entries if e["event_type"] == "trust.changed"
        ]
        ...
        corrections = [
            e for e in recent_entries if e["event_type"] == "correction.received"
        ]
        ...
        return {
            ...
            "summary": {
                ...
                "failures": fail_count,
                "success_rate": round(success_count / total_executions, 4)
    ...
    async def get_agent_summary(self, agent_id: str) -> dict:
        ...
        exec_completed = [
            e for e in entries if e["event_type"] == "execution.completed"
        ]
        ...
        total_cost = sum(
            e.get("details", {}).get("total_cost_usd", 0.0) for e in exec_completed
        )
        total_latency = sum(
            e.get("details", {}).get("total_latency_ms", 0.0) for e in exec_completed
        )
        ...
        return {
            ...
            "avg_latency_ms": round(total_latency / successes, 1)
            if successes > 0
            else 0.0,
            "corrections_received": len(corrections),
            "trust_changes": [
                {"details": tc.get("details", {}), "timestamp": tc["timestamp"]}
                for tc in trust_changes
            ],
