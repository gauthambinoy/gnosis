...
class RetentionEngine:
    ...
    def get_policy(self, workspace_id: str) -> RetentionPolicy:
        return self._policies.get(
            workspace_id, RetentionPolicy(workspace_id=workspace_id)
        )
    ...
    def calculate_expiry(self, workspace_id: str) -> dict:
        ...
        return {
            "executions_expire_before": (
                now - timedelta(days=policy.execution_days)
            ).isoformat(),
            "memories_expire_before": (
                now - timedelta(days=policy.memory_days)
            ).isoformat(),
            "audit_expire_before": (
                now - timedelta(days=policy.audit_days)
            ).isoformat(),
            "files_expire_before": (now - timedelta(days=policy.file_days)).isoformat(),
            "activities_expire_before": (
