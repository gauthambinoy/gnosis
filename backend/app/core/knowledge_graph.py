...
class KnowledgeGraphEngine:
    ...
    def add_entity(self, name: str, entity_type: str, agent_id: str = None, properties: dict = None) -> Entity:
        ...
        entity = Entity(
            id=str(uuid.uuid4()), name=name, entity_type=entity_type,
            agent_id=agent_id, properties=properties or {},
        )
    ...
    def add_relationship(self, source_name: str, target_name: str, relation_type: str, context: str = "") -> Optional[Relationship]:
        ...
        rel = Relationship(
            ...
            relation_type=relation_type, context=context[:200],
        )
    ...
    def extract_from_text(self, text: str, agent_id: str = None) -> dict:
        ...
        for match in re.finditer(self.ENTITY_PATTERNS["person"], text):
            name = match.group(1)
            if len(name.split()) == 2 and not any(w in name.lower() for w in ["the", "this", "that"]):
        ...
        for rel_type, keywords in self.RELATION_KEYWORDS.items():
            for keyword in keywords:
                ...
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    ...
                    if self._normalize(source) in self._name_to_id and self._normalize(target) in self._name_to_id:
                        rel = self.add_relationship(source, target, rel_type, context=match.group(0))
                        if rel:
                            rels_found.append(rel)
        
        return {"entities_extracted": len(entities_found), "relationships_extracted": len(rels_found)}
    ...
    def get_graph(self, agent_id: str = None) -> dict:
        ...
        if agent_id:
            ...
            edges = [r for r in self._relationships.values() if r.source_id in entity_ids or r.target_id in entity_ids]
        else:
            nodes = list(self._entities.values())
            edges = list(self._relationships.values())
        
        return {
            "nodes": [{"id": n.id, "name": n.name, "type": n.entity_type, "mentions": n.mentions, "properties": n.properties} for n in nodes],
            "edges": [{"id": e.id, "source": e.source_id, "target": e.target_id, "type": e.relation_type, "weight": e.weight} for e in edges],
    ...
    def get_entity_neighborhood(self, entity_name: str, depth: int = 1) -> dict:
        ...
        for _ in range(depth):
            ...
            nodes.extend([self._entities[eid] for eid in next_frontier if eid in self._entities])
            frontier = next_frontier
        
        return {
            "nodes": [{"id": n.id, "name": n.name, "type": n.entity_type, "mentions": n.mentions} for n in nodes],
            "edges": [{"id": e.id, "source": e.source_id, "target": e.target_id, "type": e.relation_type, "weight": e.weight} for e in edges],
        }
