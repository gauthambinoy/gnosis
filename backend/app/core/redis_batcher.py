...
class RedisBatcher:
    ...
    async def execute_batch(
        self, operations: List[tuple], redis_client=None
    ) -> List[Any]:
        ...
        try:
            ...
            self._metrics.avg_batch_size = (
                self._metrics.total_ops / self._metrics.total_batches
            )
            self._metrics.avg_batch_ms = (self._metrics.avg_batch_ms * 0.9) + (
                elapsed_ms * 0.1
            )
    ...
    async def multi_set(
        self, mapping: Dict[str, Any], ttl: int = None, redis_client=None
    ) -> bool:
