...
class MemoryEngine:
    ...
    def __init__(self):
        ...
        self._memories: dict[str, dict[str, list[MemoryEntry]]] = {}  # agent_id -> tier -> memories
        self._sensory_buffer: dict[str, list[dict]] = {}  # agent_id -> recent events (max 100)
    ...
    async def store(self, agent_id: str, tier: str, content: str, metadata: dict | None = None) -> MemoryEntry:
        ...
        entry = MemoryEntry(
            id=mem_id, agent_id=agent_id, tier=tier, content=content,
        ...
        store.add(mem_id, embedding, {"content": content, "tier": tier, **(metadata or {})})
    ...
    async def store_correction(self, agent_id: str, original_action: str, correction: str, context: dict) -> MemoryEntry:
        ...
        return await self.store(
            agent_id=agent_id, tier="correction", content=content,
            metadata={"original": original_action, "correction": correction, "context": context},
        )
    ...
    async def retrieve_context(self, agent_id: str, trigger_data: dict) -> MemoryContext:
        ...
        corrections = [self._result_to_entry(r, agent_id) for r in results if r.metadata.get("tier") == "correction"]
        episodic = [self._result_to_entry(r, agent_id) for r in results if r.metadata.get("tier") == "episodic"]
        semantic = [self._result_to_entry(r, agent_id) for r in results if r.metadata.get("tier") == "semantic"]
        procedural = [self._result_to_entry(r, agent_id) for r in results if r.metadata.get("tier") == "procedural"]

        # Recent from sensory buffer
        recent = self._sensory_buffer.get(agent_id, [])[-10:]
        recent_entries = [
            MemoryEntry(id=f"sensory-{i}", agent_id=agent_id, tier="sensory", content=str(e))
    ...
    async def get_agent_memories(self, agent_id: str, tier: str | None = None, limit: int = 50) -> list[MemoryEntry]:
    ...
    def stats(self, agent_id: str) -> dict:
        ...
        return {"vectors": vectors, "sensory_buffer": sensory_count, "embeddings": embedding_stats}
