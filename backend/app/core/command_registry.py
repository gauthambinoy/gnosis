...
BUILTIN_COMMANDS = [
    Command("cmd-palette", "Open Command Palette", "Quick access to all commands", "Ctrl+K", "navigation"),
    ...
    Command("nav-knowledge", "Go to Knowledge Base", "Access knowledge", "G K", "navigation"),
    ...
    Command("new-pipeline", "Create New Pipeline", "Build a workflow", "Shift+N", "actions"),
    ...
    Command("shortcuts", "Show All Shortcuts", "List keyboard shortcuts", "Shift+?", "help"),
    ...
    Command("fullscreen", "Toggle Fullscreen", "Enter/exit fullscreen", "F11", "appearance"),
    Command("copy-id", "Copy Current ID", "Copy item ID to clipboard", "Ctrl+Shift+C", "actions"),
    Command("export", "Export Data", "Export current view", "Ctrl+E", "actions"),
    Command("notifications", "Toggle Notifications", "Show/hide notifications", "Ctrl+Shift+N", "actions"),
]
...
class CommandRegistry:
    ...
    def search_commands(self, query: str) -> List[dict]:
        q = query.lower()
        return [asdict(c) for c in self._commands.values() if q in c.label.lower() or q in c.description.lower()]
