...
class PromptOptimizer:
    ...
    OPTIMIZATION_RULES = [
        ...
        },
        {
            ...
            "check": lambda p: (
                "format" not in p.lower()
                and "json" not in p.lower()
                and "list" not in p.lower()
            ),
        ...
        },
        {
            ...
            "check": lambda p: (
                any(w in p.lower() for w in ["how", "explain", "why", "analyze"])
                and "step" not in p.lower()
            ),
        ...
        for rule in self.OPTIMIZATION_RULES:
            ...
            if "patterns_remove" in rule:
                for pattern in rule["patterns_remove"]:
                    new_optimized = re.sub(
                        pattern, "", optimized, flags=re.IGNORECASE
                    ).strip()
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "avg_quality_boost": round(avg_boost, 1),
        }
