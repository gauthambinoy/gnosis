...
class ConfidenceEngine:
    ...
    def score(
        ...
        # Memory match score
        if memory_results > 0:
            memory_score = min(100, memory_max_score * 100 * (1 + min(memory_results, 5) / 10))
            if has_corrections:
        ...
        length_factor = min(1.0, response_length / 200)  # Longer = more detailed = more certain
        model_score = 50 + length_factor * 30 + (trust_level * 5)
        
        # Context coverage
        context_ratio = min(1.0, context_tokens / max_context_tokens) if max_context_tokens > 0 else 0
