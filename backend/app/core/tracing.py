...
class Tracer:
    ...
    def start_trace(self, operation: str) -> Span:
        ...
        span = Span(trace_id=trace_id, span_id=span_id, parent_id=None, 
                     operation=operation, start_time=time.perf_counter())
        ...
        for span in reversed(self.spans):
            if span.trace_id not in traces:
                traces[span.trace_id] = {
                    ...
                    "root_operation": span.operation if not span.parent_id else traces.get(span.trace_id, {}).get("root_operation", span.operation),
                    "spans": [],
                    "total_duration_ms": 0,
                }
