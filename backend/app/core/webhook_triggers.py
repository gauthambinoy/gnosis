...
class WebhookTriggerManager:
    ...
    def create_trigger(
        self, agent_id: str, name: str, allowed_sources: list = None
    ) -> WebhookTrigger:
    ...
    def verify_signature(self, trigger_id: str, payload: bytes, signature: str) -> bool:
        ...
        if not trigger:
            return False
        expected = hmac.new(
            trigger.secret.encode(), payload, hashlib.sha256
        ).hexdigest()
        return hmac.compare_digest(expected, signature)

    async def invoke(
        self, trigger_id: str, payload: dict, headers: dict, source_ip: str
    ) -> WebhookInvocation:
        trigger = self._triggers.get(trigger_id)
        if not trigger:
    ...
    def get_invocations(
        self, trigger_id: str = None, limit: int = 50
