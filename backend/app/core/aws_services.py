...
class AWSServices:
    ...
    async def generate_presigned_url(
        self, s3_uri: str, expires_in: int = 3600
    ) -> Optional[str]:
        if not self._available or not s3_uri.startswith("s3://"):
            return None
        try:
    ...
    async def send_execution_task(
        ...
        try:
            ...
            resp = self._sqs.send_message(
                QueueUrl=queue_url,
                MessageBody=msg,
                MessageGroupId=agent_id if ".fifo" in queue_url else None,
            )
    ...
    async def send_webhook_event(self, webhook_id: str, payload: dict) -> Optional[str]:
        ...
        try:
            msg = json.dumps(
                {"webhook_id": webhook_id, "payload": payload, "timestamp": time.time()}
            )
    ...
    async def get_execution_history(self, agent_id: str, limit: int = 20) -> list[dict]:
        ...
        try:
            table = self._dynamodb.Table(table_name)
            resp = table.query(
                KeyConditionExpression=boto3.dynamodb.conditions.Key("agent_id").eq(
                    agent_id
                ),
    ...
    def get_status(self) -> dict:
        return {
            ...
            "dynamodb_table": self.settings.dynamodb_execution_table
            or "(not configured)",
        }
