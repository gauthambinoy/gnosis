...
class WebhookManager:
    ...
    async def deliver(self, event_type: str, payload: dict):
        ...
        matching = [s for s in self.subscriptions if s["active"] and event_type in s["events"]]
        tasks = [self._deliver_one(sub, event_type, payload) for sub in matching]
        if tasks:
            await asyncio.gather(*tasks, return_exceptions=True)
    
    async def _deliver_one(self, sub: dict, event_type: str, payload: dict, attempt: int = 1):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(sub["url"], data=body, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
