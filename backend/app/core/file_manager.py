...
class FileManager:
    ...
    async def upload(
        ...
        if kind is not None:
            ...
            if detected_ext != ext:
                ...
                if (
                    EXTENSION_ALIASES.get(ext) != detected_ext
                    and EXTENSION_ALIASES.get(detected_ext) != ext
                ):
                    logger.warning(
                        f"MIME mismatch rejected: claimed={ext}, detected={detected_ext} for {original_name}"
                    )
                    raise ValueError(
                        f"File type mismatch: extension is {ext} but content is {detected_ext}"
                    )
        ...
        logger.info(
            f"File uploaded ({storage}): {file_id} ({original_name}, {len(content)} bytes)"
        )
        return record
    ...
    async def read_text(self, file_id: str) -> Optional[str]:
        ...
        if self._files.get(file_id, None) and self._files[file_id].filename.startswith(
            "s3://"
        ):
