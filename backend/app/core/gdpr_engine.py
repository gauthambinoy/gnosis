...
class GDPREngine:
    ...
    async def execute_erasure(self, request_id: str) -> ErasureRequest:
        ...
        try:
            ...
            if erase_all or "agents" in categories:
                results["agents"] = {
                    "status": "queued",
                    "note": "Agent data marked for deletion",
                }
            if erase_all or "executions" in categories:
                results["executions"] = {
                    "status": "queued",
                    "note": "Execution history marked for deletion",
                }
            if erase_all or "memories" in categories:
                results["memories"] = {
                    "status": "queued",
                    "note": "Memory entries marked for deletion",
                }
            if erase_all or "files" in categories:
                results["files"] = {
                    "status": "queued",
                    "note": "Uploaded files marked for deletion",
                }
            if erase_all or "audit_logs" in categories:
                results["audit_logs"] = {
                    "status": "queued",
                    "note": "Audit logs marked for anonymization",
                }
            if erase_all or "profile" in categories:
                results["profile"] = {
                    "status": "queued",
                    "note": "User profile marked for anonymization",
                }
            ...
            logger.info(
                f"GDPR erasure completed: {request_id}, categories: {list(results.keys())}"
            )
    ...
    def list_requests(self, user_id: str = None) -> List[dict]:
        ...
        return [
            asdict(r)
            for r in sorted(requests, key=lambda r: r.created_at, reverse=True)
        ]
    ...
    def get_user_data_inventory(self, user_id: str) -> dict:
        ...
        return {
            ...
            "data_categories": {
                "agents": {
                    "description": "AI agents created by the user",
                    "erasable": True,
                },
                "executions": {
                    "description": "Agent execution history and results",
                    "erasable": True,
                },
