...
class RetryEngine:
    ...
    def __init__(
        self, max_attempts: int = 3, base_delay: float = 2.0, max_delay: float = 60.0
    ):
    ...
    async def execute_with_retry(
        self, execution_id: str, fn: Callable, *args, **kwargs
    ) -> Any:
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                if attempt > 0:
                    logger.info(
                        f"Retry succeeded: {execution_id} on attempt {attempt + 1}"
                    )
                return result
                ...
                logger.warning(
                    f"Execution failed: {execution_id}, attempt {attempt + 1}/{self.max_attempts}: {e}"
                )
                ...
                if attempt < self.max_attempts - 1:
                    ...
                    self._retry_count += 1
                    logger.info(
                        f"Retrying {execution_id} in {delay:.1f}s (attempt {attempt + 2})"
                    )
