...
class PipelineStep:
    ...
    name: str
    order: int
    transform_input: Optional[str] = (
        None  # Jinja-like template to transform output->input
    )
...
class PipelineEngine:
        ...
        if steps:
            for i, step_data in enumerate(steps):
                ...
                if not self._validate_agent_id(agent_id):
                    raise ValueError(
                        f"Pipeline step references non-existent agent: {agent_id}"
                    )
                step = PipelineStep(
                    id=str(uuid.uuid4()),
                    agent_id=agent_id,
    ...
    def list_pipelines(self) -> List[Pipeline]:
        return sorted(
            self._pipelines.values(), key=lambda p: p.created_at, reverse=True
        )
    ...
    def add_step(
        self, pipeline_id: str, agent_id: str, name: str, **kwargs
    ...
    async def execute(
        self, pipeline_id: str, initial_input: dict = None
    ) -> PipelineRun:
        pipeline = self._pipelines.get(pipeline_id)
        if not pipeline:
        ...
        for i, step in enumerate(sorted_steps):
            ...
            try:
                ...
                # Transform input
                step_input = (
                    self._transform_input(step.transform_input, current_output)
        ...
        logger.info(
            f"Pipeline {pipeline_id} completed in {run.total_duration_ms:.1f}ms"
        )
        return run
        ...
        for attempt in range(step.max_retries + 1):
            try:
                if self._execute_agent_fn:
                    ...
                    return {
                        "output": f"[dry-run] Step '{step.name}' executed with input: {list(input_data.keys())}"
                    }
            except Exception as e:
                last_error = e
    ...
    def stats(self) -> dict:
        return {
            ...
            "completed_runs": sum(
                1 for r in self._runs.values() if r.status == PipelineStatus.COMPLETED
            ),
            "failed_runs": sum(
