...
class FlameProfiler:
    ...
    def __init__(self):
        ...
        self._active: Dict[str, dict] = {}  # execution_id -> {"profile": ..., "stack": [...], "start": ...}
    ...
    def push_span(self, execution_id: str, name: str, metadata: dict = None) -> Optional[str]:
    ...
    def list_profiles(self, agent_id: str = None, limit: int = 20) -> List[dict]:
        ...
        return [{"id": p.id, "execution_id": p.execution_id, "agent_id": p.agent_id, "total_duration_ms": p.total_duration_ms, "span_count": self._count_spans(p), "created_at": p.created_at} for p in profiles[:limit]]
    ...
    def _profile_to_dict(self, profile: FlameProfile) -> dict:
        def _span_dict(span):
            return {
                "id": span.id, "name": span.name, "parent_id": span.parent_id,
                "start_ms": round(span.start_ms, 2), "end_ms": round(span.end_ms, 2),
                "duration_ms": round(span.duration_ms, 2), "metadata": span.metadata,
                "children": [_span_dict(c) for c in span.children],
            }
        return {
            "id": profile.id, "execution_id": profile.execution_id, "agent_id": profile.agent_id,
            "total_duration_ms": round(profile.total_duration_ms, 2), "created_at": profile.created_at,
